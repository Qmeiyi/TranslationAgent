...
1. **上下文长度不可控**：章节长度差异较大，容易超出模型可接受的上下文窗口。
2. **错误定位困难**：一旦翻译结果出现问题，难以精确定位错误来源。
3. **流程不可恢复**：中途失败需整体重跑，计算成本高，调试效率低。
4. **一致性约束难以落地**：术语和专名的约束粒度过粗，无法在局部修正。

基于上述问题，本项目在翻译阶段之前引入了**显式的长文本切段（Chunking）机制**，将原本以“章节”为单位的处理方式，细化为**以切段为基本翻译单元的流水线式处理流程**。

### 3.2 切段策略设计

切段策略遵循“**语义优先、长度可控、顺序稳定**”的原则，具体规则如下：
...
3. **长度约束**
   每个切段的目标长度控制在约 **800–1500 字符**，在保证上下文充分的同时避免过长输入。

4. **稳定的切段标识**
   每个切段生成唯一且可复现的 `chunk_id`，用于后续状态管理与结果回溯。
...
* **翻译单元可独立处理**：每个切段可以单独翻译、验证和修正。
* **错误精确定位**：问题可被精确定位到具体章节与切段。
* **流程可中断与恢复**：已完成切段无需重复计算，支持断点重启。
* **一致性控制更精细**：术语约束可针对具体上下文生效。

### 3.4 上下文衔接策略

切段虽然提升了可控性，但也可能引入上下文割裂的问题。为此，系统在翻译阶段为每个切段提供**局部上下文补充信息**，包括但不限于：

* **前一切段的简要摘要**
* **前一切段译文的末尾句子**

这些上下文信息在翻译提示中作为辅助输入，用于帮助模型理解当前切段在章节中的位置和叙事连续性。
...
* **与术语提取模块**：
  术语提取既可基于全书文本，也可结合切段结果定位术语首次出现位置。
* **与翻译模块（TEaR）**：
  每个切段独立进入 TEaR 翻译—验证—修正流程。
* **与流程调度模块**：
  切段作为 Workflow 中的核心状态单元，支持进度追踪与持久化存储。

通过上述设计，长文本翻译被转化为一系列结构清晰、可控的子任务，为后续的一致性控制与质量优化奠定了基础。
